class Snippets < Test::Unit::TestCase
  ...
  def wait_for_element_present locator
    @wd_wait.until { @browser.find_element locator }
  ...
  def pass_in_object_check_download_report_feature element, locator
    ...
    # Wait for the Download to... to load in document
    self.wait_for_element_present locator
    download_link = @browser.find_element locator
  ...
  def wait_for_element_text locator
    result = @wd_wait.until {
      result = @browser.find_element(locator).text
  ...
  def wait_for_text locator
    @wd_wait.until { 
      results = @browser.find_element(locator).text 
  ...
  def wait_for_and_submit locator 
    @wd_wait.until { @browser.find_element locator }
    @browser.find_element( locator ).submit
  end
  ...
  def get_elements_by_tag_name tags_container, tag
    element = @browser.find_element( tags_container )
  ...
  def loop_through_options select_locator, button_locator, reporter_locator
    ...
    select_element = @browser.find_element select_locator
    options = select_element.find_elements :tag_name => "option"

    self.wait_for_element_present button_locator
    button_element = @browser.find_element button_locator 

    options.each do |option|
      option.click
      button_element.click
      self.wait_for_element_present reporter_locator
    end
  end
  ...
  def loop_run_report_through_checkbox_filter not_in, container, button, reporter
    ...
    @wd_wait.until { @browser.find_element button }
    button = @browser.find_element button
  ...
  def loop_run_report_through_checkbox_filters container, button, reporter
    ...
    elements.each { |item| 
      ...
      @wd_wait.until { @browser.find_element reporter }
      # Unselect filter
      item.click
    }
  end
  ...
  def click_through_reporter_column_headings reporter_container
    @wd_wait.until { @browser.find_element reporter_container }
  ...
  def get_attribute_value locator, attribute
    @browser.find_element( locator ).attribute( attribute )
  end
  ...
  def assert_errors_are_not_present
    @wd_wait.until { @browser.execute_script("return document.body") }
